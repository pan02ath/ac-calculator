...
st.title("Υπολογιστής ενεργειακών απαιτήσεων για την επιλογή κλιματιστικού")
...
with c5:
    ...
    μη_θερμαινόμενοι = st.selectbox("Επαφή με μη θερμαινόμενους χώρους", list(ΜΗ_ΘΕΡΜΑΙΝΟΜΕΝΟΙ.keys()))
...
if st.button("Υπολογισμός"):
    try:
        ...
        if περιστασιακή or αθόρυβη or βόρειος:
            st.warning("⚠️ Τα αποτελέσματα έχουν προσαυξηθεί βάσει των επιλογών 'Χρήσης', 'Αθόρυβης λειτουργίας' ή 'Προσανατολισμού'.")

        # Summary Report
        st.subheader("Σύνοψη Δεδομένων & Αποτελεσμάτων")
        report = f"""ΑΝΑΦΟΡΑ ΕΝΕΡΓΕΙΑΚΩΝ ΑΠΑΙΤΗΣΕΩΝ...""" # [As defined previously]
        st.code(report, language="text")

        # --- THE DISCLAIMER BELONGS HERE ---
        st.markdown("---")
        st.caption("""
        **Αποποίηση Ευθύνης:** Ο παρών υπολογισμός αποτελεί εκτίμηση βάσει των δεδομένων που εισήχθησαν και προορίζεται για ενημερωτική χρήση. 
        Οι πραγματικές απώλειες και απαιτήσεις μπορεί να διαφέρουν ανάλογα με τις ιδιαιτερότητες της κατασκευής και τις τοπικές συνθήκες. 
        Για την οριστική μελέτη, επιλογή και εγκατάσταση του συστήματος κλιματισμού, απαιτείται αυτοψία και συμβουλή από αδειούχο μηχανολόγο ή πιστοποιημένο ψυκτικό.
        """)
